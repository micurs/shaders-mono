...
const buildScene = async (gpu: Gpu): Promise<Scene> => {
  const color1: WebGPU.RGBAColor = [0.8, 0.8, 0.1, 1.0]; // WebGPU.styleColorToVec(window.getComputedStyle(gpu.canvas).color);
  const color2: WebGPU.RGBAColor = [0.5, 0.5, 1.0, 1.0];
  const color3: WebGPU.RGBAColor = [0.8, 0.3, 1.0, 1.0];

  const cube = WebGPU.cubeTriMesh(Transform.rotationX(Math.PI / 3).translation(-1.8, 0, 0), color3);
  const cylinder = WebGPU.cylinderTriMesh(12, color2, Transform.rotationX(Math.PI * 0.7).translation(1.8, 0, 0));
